...
class DobotApiDashboard(DobotApi):
    ...
    def InverseSolution(self, x, y, z, rx, ry, rz):
        """
        逆解
        """
        string = "InverseSolution({:f},{:f},{:f},{:f},{:f},{:f},0,0)".format(
            x, y, z, rx, ry, rz)
        self.send_data(string)
        return self.wait_reply()
    
    def PositiveSolution(self, j1, j2, j3, j4, j5, j6):
        """
        正解
        """
        string = "PositiveSolution({:f},{:f},{:f},{:f},{:f},{:f},0,0)".format(
            j1, j2, j3, j4, j5, j6)
        self.send_data(string)
        return self.wait_reply()
